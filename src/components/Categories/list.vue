<template>
  <div v-if="this.$route.name=='Categories'" class="content d-flex flex-column flex-column-fluid" id="kt_content">
    <!--begin::Toolbar-->
...
<!--                </th>-->
              ...
              <tr v-for="(list ,index) in filteredList" :key="list">
                ...
                <th >{{$t(list.type)}}</th>
                <th>
                  <router-link :to="{name: 'CategoriesUpdate' , params: {id:list.id} }" class="btn btn-info btn-circle">
                  {{$t('edit')}} <i class="fas fa-edit"></i>
                  </router-link>
                  <delete  :id="list.id"  link="admin/category/delete" method="POST" @deleted="remove(index)" ></delete>
...
export default {
  ...
  methods: {
    getResults(page = 1) {
      this.data.page = page;
      this.$axios.get('admin/category?page='+this.data.page)
          .then(response => {
