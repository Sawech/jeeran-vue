            ...
            >
              ...
              <tbody class="fw-bold text-gray-600">
                ...
                <tr v-for="(list, index) in filteredList" :key="list">
                  ...
                  <th>{{ list.email }}</th>
                  <th>{{ list.mobile }}</th>
                  <th v-if="list.last_order != null">
                    <router-link
                      :to="{
                        name: 'OrderShow',
                        params: { id: list.last_order.id },
                      }"
                      >{{ list.last_order.id }}</router-link
                    >
                  </th>
                  <th v-if="list.last_order == null"></th>

                  <th>{{ $t(list.status) }}</th>

                  <th>
                    <router-link
                      :to="{ name: 'UserUpdate', params: { id: list.id } }"
                    ...
                    <router-link
                      :to="{ name: 'UserSize', params: { id: list.id } }"
                      class="btn btn-warning btn-circle"
                    >
                      {{ $t("sizes") }} <i class="fas fa-user-tie"></i>
                    </router-link>
                    <delete
                      :id="list.id"
...
export default {
  ...
  methods: {
    getResults(page = 1) {
      this.data.page = page;
      this.$axios
        .post("admin/user?page=" + this.data.page, {
          type: "user",
        })
        .then((response) => {
