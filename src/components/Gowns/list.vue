<template>
  <div v-if="this.$route.name=='Gowns'" class="content d-flex flex-column flex-column-fluid" id="kt_content">
    <!--begin::Toolbar-->
        ...
        <div class="card card-flush">
          ...
          <div class="card-header align-items-center py-5 gap-2 gap-md-5">
            ...
            <div class="card-toolbar">
              <!--begin::Add customer-->
              <router-link :to="{name: 'GownsCreate'}">
...
<!--                </th>-->
              ...
              <tr v-for="(list ,index) in filteredList" :key="list">
                ...
                <th>{{$t(list.sort)}}</th>               
                <th>
                  <router-link :to="{name: 'GownsUpdate' , params: {id:list.id} }" class="btn btn-info btn-circle">
                  {{$t('edit')}} <i class="fas fa-edit"></i>
                  </router-link>
                  <delete  :id="list.id"  link="admin/size-gown/delete" method="POST" @deleted="remove(index)" ></delete>
...
export default {
  ...
  methods: {
    getResults(page = 1) {
      this.data.page = page;
      this.$axios.get('admin/size-gown?page='+this.data.page)
          .then(response => {
