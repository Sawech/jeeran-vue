<template>
  <div v-if="this.$route.name=='SizeUpdate'" class="content d-flex flex-column flex-column-fluid" id="kt_content">
        ...
        <form id="kt_ecommerce_add_category_form" class="form d-flex flex-column flex-lg-row" data-kt-redirect="../../demo1/dist/apps/ecommerce/catalog/categories.html">
          ...
          <div class="d-flex flex-column flex-row-fluid gap-7 gap-lg-10">
            ...
            <div class="d-flex justify-content-end">
              <router-link :to="{name: 'Size'}" class="btn btn-light me-5">
                {{ $t('cancel') }}
              </router-link>
...
export default {
  ...
  methods: {
    ...
    edit() {
      this.$axios.post('admin/size-type/fetch?id='+this.id)
          .then(response => {
    ...
    update() {
      ...
      this.$axios.post('/admin/size-type/edit',this.form)
          .then(response => {
