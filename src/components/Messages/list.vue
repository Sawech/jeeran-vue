<template>
  <div v-if="this.$route.name=='Messages'" class="content d-flex flex-column flex-column-fluid" id="kt_content">
    <!--begin::Toolbar-->
...
<!--                </th>-->
              ...
              <tr v-for="(list ,index) in filteredList" :key="list">
                ...
                <th>
                  <router-link :to="{name: 'MessageShow' , params: {id:list.id} }" class="btn btn-info btn-circle">
                  {{$t('contentMessage')}} <i class="fas fa-eye"></i>
                  </router-link>
                  <delete  :id="list.id"  link="admin/dashboard/message/delete" method="POST" @deleted="remove(index)" ></delete>
...
export default {
  ...
  methods: {
    getResults(page = 1) {
      this.data.page = page;
      this.$axios.get('admin/dashboard/messages?page='+this.data.page)
          .then(response => {
