<template>
  <div v-if="this.$route.name=='SizeTypeCategoriesCreate'" class="content d-flex flex-column flex-column-fluid" id="kt_content">
            ...
            <div class="card card-flush py-4">
              ...
              <div class="card-body pt-0">
                <div class="mb-10 fv-row">
                  {{$t('categories')}}
                  <select class="form-select"    v-model="form.category_id" name="condition_type" data-placeholder="Select an option" data-kt-ecommerce-catalog-add-category="condition_type">
                    <option v-for="category in categories" :value="category.id" :key="category" >{{category.name}}</option>
...
export default {
  ...
  methods: {
    ...
    getCategories() {
      this.$axios.get('admin/all-category')
          .then(response => {
