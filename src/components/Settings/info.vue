<template>
  <div v-if="this.$route.name=='Info'" class="content d-flex flex-column flex-column-fluid" id="kt_content">
...
<!--                  </ul>           -->
              ...
              <div class="card-body pt-0">
                <div class="mb-10 fv-row">
                  <div v-for="(lang,index) in form" :key="index">
                    {{$t(lang.attribute)}}
                    <input type="text" name="category_name" v-model="lang.value" class="form-control mb-2" :placeholder="$t(lang.attribute)" />
                  </div>
                </div>
              </div>
            </div>
  ...
  methods: {
    ...
    edit() {
      this.$axios.get('admin/dashboard')
          .then(response => {
            this.form = response.data.data.siteInfo;
            this.loading = 0;
          });
    },
    update() {
      ...
      this.$axios.post('admin/dashboard/edit-site-info',data)
          .then(response => {
